...
SELECT  Adv_Type,
SUM(CASE WHEN Action='Order' AND Adv_Type='A' THEN Conversion ELSE 0 END)*1.0/(SELECT SUM(CASE WHEN Adv_Type='A' THEN Conversion ELSE 0 END) 
FROM order1) AS Conversion_Rate INTO table1
FROM order1
GROUP BY Adv_Type
UNION --ALL
SELECT Adv_Type, SUM(CASE WHEN Action='Order' AND Adv_Type='B' THEN Conversion ELSE 0 END)*1.0/(SELECT SUM(CASE WHEN Adv_Type='B' THEN Conversion ELSE 0 END) 
...
SELECT SUM(CASE WHEN Adv_Type='B' THEN Conversion ELSE 0 END) 
FROM order1
